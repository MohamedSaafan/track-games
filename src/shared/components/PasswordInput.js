    ...
    <>
      ...
      {showRequirements && requirements && (
        <div className="mb-3">
          <p>Requirements:</p>
          <Row className="ms-3">
            <Col md={4} className={`my-1 ${passwordRequirements.isLength ? "text-success" : ""}`}>
              8 characters
            </Col>
            <Col md={4} className={`my-1 ${passwordRequirements.hasUpperCase ? "text-success" : ""}`}>
              1 uppercase
            </Col>
            <Col md={4} className={`my-1 ${passwordRequirements.hasLowerCase ? "text-success" : ""}`}>
              1 lowercase
            </Col>
            <Col md={4} className={`my-1 ${passwordRequirements.hasNumber ? "text-success" : ""}`}>
              1 number
            </Col>
            <Col md={4} className={`my-1 ${passwordRequirements.hasSymbol ? "text-success" : ""}`}>
              1 symbol
            </Col>
            <Col md={4} className={`my-1 ${passwordRequirements.hasSpace ? "text-success" : ""}`}>
              No spaces
            </Col>
          </Row>
        </div>
